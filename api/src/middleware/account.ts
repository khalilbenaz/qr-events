...
async function load(c: Parameters<MiddlewareHandler<AppEnv>>[0]) {
  const row = await c.env.DB.prepare('SELECT role, status, plan FROM organizers WHERE id = ?')
    .bind(c.get('organizerId'))
    .first<{ role: OrganizerRole; status: OrganizerStatus; plan: string | null }>();
...
export const requireAdmin: MiddlewareHandler<AppEnv> = async (c, next) => {
  const row = await load(c);
  if (row.role !== 'admin')
    throw new ApiError(403, 'forbidden', "Accès réservé à l'administrateur");
  await next();
};
