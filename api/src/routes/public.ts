...
async function resolvePublicEvent(
  ...
  const ev = await c.env.DB.prepare(
    ...
    .bind(orgSlug, eventSlug)
    .first<EventRow & { organizer_name: string }>();
...
pub.get('/org/:orgSlug', async (c) => {
  const org = await c.env.DB.prepare('SELECT id, name, slug FROM organizers WHERE slug = ?')
    .bind(c.req.param('orgSlug'))
...
pub.post('/event/:orgSlug/:eventSlug/register',
  ...
  const existing = await c.env.DB.prepare(
    ...
    .bind(ev.id, email)
    .first<TicketRow>();
  if (existing) throw new ApiError(409, 'already_registered', 'Déjà inscrit avec cet email');
  ...
  if (allowed.length > 0) {
    ...
    const match =
      allowed.find((x) => x.name.toLowerCase() === chosen.toLowerCase()) ??
    ...
    if (match.mode === 'none')
      throw new ApiError(
        403,
        'registration_closed',
        `La catégorie « ${match.name} » n'est pas ouverte à l'inscription`,
      );
    category = match.name;
    mode = match.mode;
  } else {
  ...
  await c.env.DB.prepare(
    ...
    .bind(id, ev.id, name, email, category, qr_token, status)
    .run();
...
pub.get('/t/:token/qr', async (c) => {
  ...
  const t = await c.env.DB.prepare('SELECT status FROM tickets WHERE id = ? AND qr_token = ?')
    .bind(ticketId, token)
    .first<{ status: string }>();
  if (!t || t.status === 'revoked') throw new ApiError(404, 'not_found', 'Billet introuvable');
