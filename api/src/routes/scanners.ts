...
scannersAdmin.post('/events/:eventId/scanners',
  ...
  if (cat) {
    ...
    if (!match)
      throw new ApiError(400, 'invalid_category', 'Catégorie inconnue pour cet événement');
    cat = match;
  }
  ...
  await c.env.DB.prepare(
    ...
    .bind(id, ev.id, name.trim(), code, cat)
    .run();
...
scannersAdmin.delete('/scanners/:id', async (c) => {
  const row = await c.env.DB.prepare(
    ...
    .bind(c.req.param('id'), c.get('organizerId'))
    .first<ScannerRow>();
...
scannerAuth.post('/login',
  ...
  const b = c.req.valid('json');
  const code = b.access_code.trim().toUpperCase();
  ...
  const s = await c.env.DB.prepare(
    ...
    .bind(code)
    .first<ScannerRow & { event_name: string; event_status: string }>();
