...
interface Row {
  id: string; email: string; name: string; slug: string;
  role: string; status: string; plan: string | null;
  created_at: string; events_count: number;
}
...
export default function AdminPage() {
  ...
  const load = () => api<Row[]>("/admin/organizers").then(setRows).catch(() => setRows([]));
  useEffect(() => { if (organizer?.role === "admin") load(); }, [organizer]);

  if (organizer?.role !== "admin")
    return <Empty>Accès réservé à l'administrateur.</Empty>;
  ...
  return (
    <>
      ...
      <p className="muted" style={{ marginTop: -6 }}>
        ...
        {pending.length > 0 && <> · <strong style={{ color: "var(--amber)" }}>{pending.length} en attente</strong></>}
      </p>

      <div className="table-wrap" style={{ marginTop: 16 }}>
        <table>
          <thead><tr>
            <th>Organisateur</th><th>Statut</th><th>Offre</th><th>Évén.</th><th>Inscrit</th><th></th>
          </tr></thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                <td>
                  {r.name}{r.role === "admin" && <span className="badge violet" style={{ marginLeft: 6 }}>admin</span>}
                ...
                <td>
                  {r.role !== "admin" && (
                    <div className="row" style={{ gap: 6, justifyContent: "flex-end" }}>
                      {r.status === "pending" && (
                        <>
                          <select value={sel[r.id] ?? "discovery"}
                            onChange={(e) => setSel((s) => ({ ...s, [r.id]: e.target.value }))}
                            style={{ width: "auto", padding: "5px 8px" }}>
                            {PLANS.map((p) => <option key={p.slug} value={p.slug}>{p.label}</option>)}
                          </select>
                          <button className="btn sm primary" onClick={() => approve(r.id)}>Valider</button>
                        </>
                      )}
                      {r.status !== "suspended"
                        ? <button className="btn sm danger" onClick={() => suspend(r.id)}>Suspendre</button>
                        : <button className="btn sm primary" onClick={() => approve(r.id)}>Réactiver</button>}
                    </div>
                  )}
                </td>
              </tr>
