...
interface OrgEvent {
  ...
  date: string | null; location: string | null; cover_image_url: string | null;
  theme: string | null; capacity: number | null; used: number;
}
interface OrgData { organizer: string; slug: string; events: OrgEvent[]; }
...
export default function OrgEvents() {
  ...
  useEffect(() => {
    api<OrgData>(`/public/org/${orgSlug}`, { auth: false }).then(setData).catch(() => setNotFound(true));
  }, [orgSlug]);

  if (notFound)
    return <PlainLayout><div className="empty"><h1>Organisateur introuvable</h1></div></PlainLayout>;
  if (!data) return <PlainLayout><Spinner /></PlainLayout>;

  return (
    <PlainLayout>
      <div className="hero reveal" style={{ background: themeGradient("concert"), color: "#fff", minHeight: 0, padding: "34px" }}>
              ...
              <Link key={e.slug} to={`/${data.slug}/${e.slug}`} className="poster card-link">
                <div className="poster-img" style={{
                  backgroundImage: e.cover_image_url
                    ? `url("${e.cover_image_url}")` : themeGradient(e.theme),
                }}>
