...
export default function PublicHome() {
  ...
  const flatList = useMemo(() => {
    ...
    arr.sort(
      sort === 'pop'
        ? (a, b) => fillRatio(b) - fillRatio(a) || b.used - a.used
  ...
  const sections = useMemo(() => {
    ...
    return [
      ...
      {
        title: '🗓️ Ce mois-ci',
        items: byDate.filter((e) => timeOf(e) > now + 7 * DAY && timeOf(e) <= now + 30 * DAY),
      },
    ...
    <PlainLayout>
      ...
      <div className="row wrap" style={{ gap: 10, margin: '4px 0 16px' }}>
        ...
        <div className="row" style={{ gap: 6 }}>
          <button
            className={`chip-filter ${sort === 'date' ? 'on' : ''}`}
            onClick={() => setSort('date')}
          >
            Date
          </button>
          <button
            className={`chip-filter ${sort === 'pop' ? 'on' : ''}`}
            onClick={() => setSort('pop')}
          >
            🔥 Populaires
          </button>
      ...
      {presentThemes.length > 0 && (
        <div className="row wrap" style={{ gap: 8, marginBottom: 20 }}>
          <button
            className={`chip-filter ${!theme ? 'on' : ''}`}
            onClick={() => {
              setTheme(null);
              setLimit(PAGE);
            }}
          ...
          {presentThemes.map((t) => (
            <button
              key={t.slug}
              onClick={() => {
                setTheme(theme === t.slug ? null : t.slug);
                setLimit(PAGE);
              }}
              className={`chip-filter ${theme === t.slug ? 'on' : ''}`}
              style={
                theme === t.slug
                  ? { background: themeGradient(t.slug), color: '#fff', borderColor: 'transparent' }
                  : undefined
              }
            >
              {t.emoji} {t.label}
  ...
  return (
    <Link to={`/${e.organizer_slug}/${e.slug}`} className="poster card-link">
      ...
      <div className="poster-body">
        ...
        <p className="poster-meta" style={{ opacity: 0.8 }}>
          par {e.organizer}
        </p>
