    ...
    <div className="grid" style={{ gridTemplateColumns: '1fr', gap: 16 }}>
      <div className="card">
        <div className="row wrap">
          ...
          {ev.status !== 'published' ? (
            <button className="btn primary" onClick={() => setStatus('published')}>
              Publier
            </button>
          ) : (
      ...
      <form className="card" onSubmit={save} style={{ maxWidth: 620 }}>
        ...
        <Field label="Date & heure">
          <input
            type="datetime-local"
            value={f.date?.replace(' ', 'T').slice(0, 16)}
        ...
        <Field label="Thème de l'événement" hint="Ambiance visuelle de la page publique.">
          ...
          <div
            style={{ height: 8, borderRadius: 6, marginTop: 8, background: themeGradient(f.theme) }}
          />
        </Field>
