...
export default function RegistrationsTab({ ev }: { ev: EventRow }) {
  ...
  if (ev.registration_mode === "none")
    return <Empty>Cet événement est en mode « {MODE_LABELS.none} ». Aucune inscription publique.</Empty>;
    ...
    <div className="stack">
      {ev.registration_mode === "approval" && (
        <div className="card">
          ...
          {pending.length === 0 ? <p className="muted">Rien à valider.</p> : (
            <div className="table-wrap">
              <table>
                <thead><tr><th>Nom</th><th>Email</th><th>Demandé</th><th></th></tr></thead>
                <tbody>
                  {pending.map((t) => (
                    <tr key={t.id}>
                      <td>{t.holder_name}</td>
                      ...
                      <td>
                        <div className="row" style={{ gap: 6, justifyContent: "flex-end" }}>
                          <button className="btn sm primary" onClick={() => act(t, "approve")}>Approuver</button>
                          <button className="btn sm danger" onClick={() => act(t, "revoke")}>Refuser</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <div>
        ...
        {others.length === 0 ? <Empty>Aucune inscription validée pour l'instant.</Empty> : (
          <div className="table-wrap">
            <table>
              <thead><tr><th>Nom</th><th>Email</th><th>Catégorie</th><th>Statut</th><th>Date</th></tr></thead>
              <tbody>
                {others.map((t) => (
                  <tr key={t.id}>
                    <td>{t.holder_name}</td>
