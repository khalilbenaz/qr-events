...
export default function StatsTab({ ev }: { ev: EventRow }) {
  ...
  const load = () =>
    api<EventStats>(`/events/${ev.id}/stats`)
      .then(setS)
      .catch(() => {});
  useEffect(() => {
    load();
  ...
  return (
    <div className="stack">
      <div className="grid cols-3">
        ...
        <Stat
          num={`${s.entries.fillPercent}%`}
          label={ev.capacity ? `Rempli (cap. ${ev.capacity})` : 'Rempli'}
        />
      </div>
        ...
        <div className="card">
          <div className="row" style={{ marginBottom: 8 }}>
            ...
            <span className="muted">
              {s.entries.count} / {ev.capacity}
            </span>
          </div>
          <div className="progress">
            <div style={{ width: `${Math.min(100, s.entries.fillPercent)}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="card">
        ...
        {s.hourly.length === 0 ? (
          <Empty>Aucune entrée enregistrée.</Empty>
        ) : (
          <div className="bars">
            {s.hourly.map((h) => (
              <div
