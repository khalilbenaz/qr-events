...
export default function TicketsTab({ ev }: { ev: EventRow }) {
  ...
  const load = () =>
    api<TicketRow[]>(`/events/${ev.id}/tickets`).then(setTickets).catch(() => setTickets([]));
  ...
  const exportCsv = () => {
    if (!tickets) return;
    downloadCsv(`billets-${ev.slug}.csv`, tickets.map((t) => ({
      id: t.id, nom: t.holder_name ?? "", email: t.holder_email ?? "",
      categorie: t.category, statut: t.status, cree_le: t.created_at,
  ...
  return (
    <div className="stack">
        ...
        <div className="row" style={{ gap: 14, alignItems: "flex-end" }}>
          ...
          <div style={{ width: 180 }}>
            <Field label="Catégorie">
              <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="standard / vip / presse" />
            </Field>
          </div>
          <div style={{ marginBottom: 14 }}>
            <button className="btn primary" disabled={busy}>{busy ? "…" : "Générer"}</button>
          </div>
        </div>
      </form>
      ...
      <div className="row">
        ...
        <button className="btn sm" onClick={exportCsv} disabled={!tickets?.length}>Export CSV</button>
      </div>
      ...
      {tickets === null ? <Spinner /> : tickets.length === 0 ? (
        ...
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>Détenteur</th><th>Catégorie</th><th>Statut</th><th>Créé</th><th></th>
            </tr></thead>
            <tbody>
              {tickets.map((t) => (
                <tr key={t.id}>
                  <td>{t.holder_name || <span className="muted">— anonyme</span>}
                    {t.holder_email && <div className="muted" style={{ fontSize: ".8rem" }}>{t.holder_email}</div>}
                  </td>
                  ...
                  <td>
                    <div className="row" style={{ gap: 6, justifyContent: "flex-end" }}>
                      <button className="btn sm ghost" onClick={() => setQr(t)}>QR</button>
                      {t.status === "revoked"
                        ? <button className="btn sm" onClick={() => act(t, "reinstate")}>Réactiver</button>
                        : <button className="btn sm danger" onClick={() => act(t, "revoke")}>Révoquer</button>}
                    </div>
                  </td>
                </tr>
              ))}
