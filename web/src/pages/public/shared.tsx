...
export function RegistrationCard({ ev, orgSlug, eventSlug, result, setResult }: TemplateProps) {
    ...
    inner = (
      <>
        {ev.capacity != null && (
          <div style={{ marginBottom: 18 }}>
            ...
            <div className="progress">
              <div
                style={{
                  width: `${Math.min(100, ((ev.capacity - (ev.remaining ?? 0)) / ev.capacity) * 100)}%`,
                }}
              />
            </div>
          </div>
        ...
        <RegisterForm
          orgSlug={orgSlug}
          eventSlug={eventSlug}
          mode={ev.registration_mode}
          categories={selectableCategories(ev)}
          onDone={setResult}
        />
      </>
    );
  ...
  return (
    <div id="reserver" className="card panel ticket-panel">
      {inner}
    </div>
  );
}
  ...
  return (
    <form onSubmit={submit}>
          ...
          <select value={category} onChange={(e) => setCategory(e.target.value)} required>
            {categories.map((c) => (
              <option key={c.name} value={c.name}>
                {c.name}
                {c.mode === 'approval' ? ' — validation requise' : ''}
              </option>
            ))}
          </select>
        </Field>
      ...
      <button className="btn primary block" disabled={busy}>
        {busy ? 'Envoi…' : '🎟️ Réserver ma place'}
      </button>
    </form>
  );
}
    ...
    return (
      <div className="center">
        ...
        <p className="muted" style={{ fontSize: '.85rem' }}>
          Dès validation, votre billet QR apparaîtra ici. Gardez ce lien :
        </p>
        {ticketPath && (
          <Link to={ticketPath} className="btn primary block">
            Suivre mon billet
          </Link>
        )}
      </div>
  ...
  return (
    <div className="center">
      ...
      <div className="stack" style={{ marginTop: 14, gap: 8 }}>
        {result.qrUrl && (
          <a className="btn sm" href={result.qrUrl} target="_blank" rel="noreferrer">
            Ouvrir le QR
          </a>
        )}
        {ticketPath && (
          <Link to={ticketPath} className="btn sm ghost">
            Lien permanent
          </Link>
        )}
      </div>
