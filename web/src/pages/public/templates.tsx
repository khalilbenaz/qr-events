...
function Header({ ev, variant }: { ev: PublicEvent; variant: 'live' | 'sport' | 'pro' }) {
  ...
  const titleStyle: CSSProperties =
    variant === 'sport' ? { textTransform: 'uppercase', fontStyle: 'italic', fontWeight: 900 } : {};

  const body = (
    <>
      <span className="kicker">
        {th.emoji} {th.label} · par {ev.organizer}
      </span>
      ...
      {variant === 'sport' && (
        <div className="kicker" style={{ marginTop: 16, marginBottom: 8 }}>
          ⏱️ Coup d'envoi dans
        </div>
      )}
      {variant !== 'pro' && (
        <div style={{ marginTop: variant === 'sport' ? 0 : 16 }}>
  ...
  return (
    <div
      ...
      style={{
        minHeight: variant === 'pro' ? 220 : 320,
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'flex-end',
        background: themeGradient(ev.theme),
        color: '#fff',
        border: 'none',
      }}
    >
      ...
      <div className="event-grid">
        <main className="stack">
          {ev.description && (
            <div className="card">
              <h2>À propos</h2>
              <p style={{ color: 'var(--text)', whiteSpace: 'pre-wrap', margin: 0, opacity: 0.9 }}>
                {ev.description}
              </p>
            </div>
          )}
      ...
      <div className="elegant-body stack">
        {ev.description && (
          <div className="card center">
            <h2>À propos</h2>
            <p style={{ color: 'var(--text)', whiteSpace: 'pre-wrap', opacity: 0.9 }}>
              {ev.description}
            </p>
          </div>
        )}
...
const MAP: Record<string, (p: TemplateProps) => JSX.Element> = {
  concert: TemplateLive,
  fete: TemplateLive,
  festival: TemplateLive,
  mariage: TemplateElegant,
  gala: TemplateElegant,
  conference: TemplatePro,
  atelier: TemplatePro,
  expo: TemplatePro,
  sport: TemplateSport,
};
